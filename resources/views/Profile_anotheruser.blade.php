...
<body>
        ...
        <div class="modal-dialog modal-fullscreen-md-down modal-lg modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <a class="text-decoration-none d-flex flex-row user-info bd-callout my-4 p-3 rounded w-100 shadow addshadow" >
                        <img class="rounded-circle" src="../Images/user/{{$follower->image_url}}" width="60">
    ...
    <div class="modal fade" id="Following" tabindex="-1" aria-labelledby="Following" aria-hidden="true" dir="rtl">
        <div class="modal-dialog modal-fullscreen-md-down modal-lg modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title fw-bold fs-4" id="">المستخدمين التي تمت متابعتهم </h5>
                    <div>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                </div>
                <div class="modal-body">
                    ...
                    <div class="d-flex flex-row justify-content-between user-info bd-callout my-4 p-3 rounded w-100 shadow addshadow" href="" id="folow-{{$follow->id}}">
                        <a class="text-decoration-none d-flex flex-row align-self-end">
                            <img class="rounded-circle" src="../Images/user/{{$follow->image_url}}" width="60">
                            <div class="d-flex flex-column justify-content-start m-2">
                                <span class="d-block text-dark">{{$follow->name}}</span>
                                <span class="d-block text-muted">{{$follow->email}}</span>
                            </div>
                        </a>
        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-6 col-md-12 d-flex flex-column align-items-center py-5">
                    <!-- <img class="d-none d-lg-block d-xl-block " src="Images\user\{{ $user->image_url }}" width="450" alt="img"> -->
                    <img class="d-none d-lg-block d-xl-block " src="..\Images\svg\illustrations\Information.svg" width="450" alt="img">
                </div>
                <div class="col-lg-6 col-md-12 d-flex flex-column align-items-center py-5">
                    <div class="d-flex flex-column justify-content-center">
                        <img class="shadow img align-self-center rounded-circle" src="..\Images\user\{{$user->image_url}}" height="120" width="120" style="margin: 1%;" />
                        <span class="fs-4 fw-bold mt-3 align-self-center">{{$user->name}}</span>
                        <span class="fs-5 text-muted align-self-end">{{$user->email}}&nbsp;<i class="far fa-envelope" data-bs-toggle="tooltip" data-bs-placement="right" data-bs-title="البريد الإلكتروني" title="البريد الإلكتروني"></i></span>
                        <span class="fs-5 text-muted align-self-end">{{$user->phone}}&nbsp;<i class="fa-solid fa-phone" data-bs-toggle="tooltip" data-bs-placement="right" data-bs-title="رقم الجوال" title="رقم الجوال"></i></span>
                    </div>
                    <div class="row mt-4" dir="rtl">
                        <div class="col">
                            <ul class="nav justify-content-center gap-5">
                                <a class="nav-link shadowPurple text-center text-dark shadow-sm mt-3 bg-white">
                                    <div class="fs-5 fw-bold d-block">{{$user->posts->count()}}</div>المنشورات
                                </a>
                                <a class="nav-link shadowPurple text-center text-dark shadow-sm mt-3 bg-white" data-bs-toggle="modal" data-bs-target="#Folowers">
                                    <span class="fs-5 fw-bold d-block">{{$user->followers->count()}}</span>المتابعون
                                </a>
                                <a class="nav-link shadowPurple text-center text-dark shadow-sm mt-3 bg-white" data-bs-toggle="modal" data-bs-target="#Following">
                                    <span class="fs-5 fw-bold d-block follows_count" id="follows">{{$user->follows->count()}}</span>يتابع
                                </a>
                            </ul>
                        </div>
                    </div>
                    <div class="row mt-5">
                        <div class="col-12 d-flex flex-row gap-5 align-items-center">
                            ...
                            <form action="/users_home/{{$user->id}}/unfollow" method="POST">
                                {{ csrf_field() }}
                                {{ method_field('DELETE') }}
                                <button type="submit" id="delete-follow-{{ $user->id }}" class="btn border border-dark ">
                                    إلغاء المتابعة <i class="fas fa-window-close"></i>
                                </button>
                            </form>
                            @else
                            <form action="/users/{{$user->id}}/follow" method="POST">
                                {{ csrf_field() }}
                                <button type="submit" id="follow" value="{{$user->id}}" class="btn border border-dark">
                                    متابعة <i class="fa fa-square-plus"></i>
                                </button>
                            </form>
                            @endif
                            @elseif(Auth::check())
                    ...
                    <div class="row mt-4" dir="rtl">
                        <div class="col-12 d-flex flex-column align-items-center">
                            <span><i class="fa-solid fa-calendar-days"></i>&nbsp;تاريخ الانضمام&nbsp;<span class="fw-bold">{{$user->created_at->format('d/m/Y')}}</span></span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="col-12">
                    <nav>
                        <div class="nav nav-tabs" id="nav-tab" role="tablist">
                            <button class="nav-link active" id="nav-home-tab" data-bs-toggle="tab" data-bs-target="#nav-home" type="button" role="tab" aria-controls="nav-home" aria-selected="true">المنشورات</button>
                            @if(Auth::check() && Auth::user()->id == $user->id)
                                ...
                                <div class="col" id="delete-{{$post->id}}">
                                    <div class="card border card_shadow">
                                        <div class="card-header d-flex justify-content-between rounded-0 border-0">
                                            <a class="text-decoration-none link-secondary d-flex flex-row" href="/Profile_anotheruser/{{$post->user->id}}">
                                ...
                                <div class="col" id="delete-{{$post->id}}">
                                    <div class="card border card_shadow">
                                        ...
                                        <button class="btn icon-link col  mt-2 dislike {{ $dislike_color}}" data-like="{{$like_color}}" data-post_id="{{$post->id}}_d"> dislike<i class="far fa-thumbs-down" onclick="myFunction1(this)"></i><span class="dislike_count">{{$dislike_count}}</span></button> -->
                                                ...
                                                <span class="mb-1">
                                                    <i class="fa-solid fa-money-bill-wave text-muted fs-6" title="السعر" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="السعر"></i>&nbsp;
                                                    <span class="placeholder">{{$post->price}}</span>ل.س
                                                </span>
                                                <span><i class="fas fa-map-marker-alt text-muted fs-5" title="الموقع" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="الموقع"></i>&nbsp;&nbsp;
                                                    <span class="placeholder">{{$post->city}}</span>
                                                </span>
